...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Tabellen",
          ...
          ")),
                      actionButton("update_numbers", "Zahlen aktualisieren!",
                                   class = "btn-lg btn-success"),
                      conditionalPanel(condition="$('html').hasClass('shiny-busy')",
                                       tags$div("Zahlen werden aktualisiert...",
                                                id="loadmessage")),
               )
             ),
             fluidRow(
             ...
             tabsetPanel(
               tabPanel("Wöchentliche Übersicht",
    ...
    tabPanel("Karten",
             ...
             fluidRow(
               column(4, selectInput("graph_id", "Welcher Graph?",
                                     choices = list("Deutschlandkarte (Anrufe)" = "karte_anrufe",
                                                    "Deutschlandkarte (Anrufer*innen)" = "karte_anruferinnen",
                                                    "Deutschlandkarte (Erstanrufer*innen)" = "karte_erstanruferinnen"))),
               column(8, dateRangeInput("period", "Zeitraum?",
                                        format = "yyyy-mm-dd"))
             ...
             fluidRow(column(12,h5("Hinweis: Karten enthalten alle Anrufe, auch nicht zustandegekommene Verbindungen."))),
             fluidRow(
    ...
    tabPanel("Zeitreihen",
             fluidRow(column(12,h4("Abbildungen:"))),
             fluidRow(
               column(4, selectInput("ts_bundesland", "Daten für welches Bundesland?",
                ...
                column(8, dateRangeInput("ts_period", "Zeitraum?",
                                        format = "yyyy-mm-dd"),
                       h5("Hinweis: Es werden automatisch nur vollständig erfasste Kalenderwochen dargestellt."))
             ),
             fluidRow(
               column(4, checkboxInput('ts_firstcall', "Nur Erstanrufer", value = FALSE))
               ),
             fluidRow(
               column(4, checkboxInput('ts_success', "Nur zustandegekommene Verbindungen", value = TRUE))
             ),
             fluidRow(
               column(4, checkboxInput('ts_y_axis', "Starte Y-Achse bei 0", value = TRUE))
             ),
             fluidRow(
               column(4, sliderInput('ts_nr_breaks', "Häufigkeit der Datumsbeschriftungen",
                                     value = 5, min = 1, max = 20, step = 1, ticks = FALSE))
             ),
             fluidRow(
...
update_data <- function(data, path){
  ...
  d <- d %>% 
    group_by(caller) %>%
    mutate(datetime = lubridate::ymd_hms(paste(date,time)),
           firstcall = ifelse(caller %in% unique(data$caller), FALSE, 
...
server <- function(input, output, session) {
  ...
  file <- here("data/raw/current_data.csv")
  old_data <- read.csv(file)
  old_data$date <- lubridate::ymd(old_data$date)
  present_data <- reactiveValues(data = old_data)
  old_data <- NULL

  # Print last update and oldest date
  output$last_update <- renderText(paste("Letzte Aktualisierung:",
                                         max(present_data$data$date),
                                         sep = " "))

  output$oldest_date <- renderText(paste("Beginn des Datensatzes (Bezugspunkt für Erstanrufe):",
  ...
  table_day_Input <- function(){
    dplyr::arrange(create_provider_table(present_data$data,
                                         unit="day",
                                         min(present_data$data$date),
                                         max(present_data$data$date)),
                   desc(Tag))
  }
  output$table_day <- DT::renderDataTable(table_day_Input(),
                                          filter = "top",
  ...
  output$download_day <- downloadHandler(
    filename = function() {
      paste0("tagestabelle_", min(table_day_Input()$Tag), "_",
             max(table_day_Input()$Tag), ".xlsx")
    },
  ...
  output$table_week <- DT::renderDataTable(table_week_Input(),
  ...
  output$download_week <- downloadHandler(
    filename = function() {
      paste0("wochentabelle_", min(table_week_Input()$Start), "_",
             max(table_week_Input()$Ende), ".xlsx")
    },
    content = function(file) {
  ...
  output$table_month <- DT::renderDataTable(table_month_Input(),
                                            filter = list(position = "top",
                                                          clear = TRUE,
                                                          plain = TRUE),
                                            options = list(pageLength = 15,
  ...
  output$download_month <- downloadHandler(
    filename = function() {
      paste0("monatstabelle_", min(table_month_Input()$Start), "_",
             max(table_month_Input()$Ende), ".xlsx")
    },
    content = function(file) {
  ...
  output$table_year <- DT::renderDataTable(table_year_Input(),
                                          filter = list(position = "top",
                                                        clear = TRUE,
                                                        plain = TRUE),
                                          options = list(pageLength = 15,
  ...
  output$download_year <- downloadHandler(
    filename = function() {
      paste0("jahrestabelle_", min(table_year_Input()$Jahr), "_",
             max(table_year_Input()$Jahr), ".xlsx")
    },
    content = function(file) {
  ...
  table_herkunft_Input <- function(){
    create_herkunft_table(present_data$data,
                          min(present_data$data$date),
                          max(present_data$data$date))
  }

  ## Table Herkunft ----------------------------------------------------------
  output$table_herkunft <- DT::renderDataTable(table_herkunft_Input(),
                                               filter = list(position = "top",
                                                             clear = TRUE,
                                                             plain = TRUE),
                                               options = list(pageLength = 15,
  ...
  output$download_herkunft <- downloadHandler(
    filename = function() {
      paste0("herkunftstabelle_", min(table_herkunft_Input() %>% filter(Jahr != "Gesamt") %>% pull(Start) %>% as.Date.character()), "_",
             max(table_herkunft_Input() %>% filter(Jahr != "Gesamt") %>% pull(Ende) %>% as.Date.character()), ".xlsx")
    },
    content = function(file) {
  ...
  plotInput_ts <- function(){
    silbRnetz:::plot_CallsOverTime(present_data$data,
  ...
  output$plot <- renderPlot(print(plotInput()),
                            ...
                            height = 1000,
                            width = 725)
  output$plot_ts <- renderPlot(print(plotInput_ts()),
                               res = 120,
                               height = 725,
                               width = 1000)
  ...
  output$download_plot <- downloadHandler(
    ...
    content = function(file) {
      ...
      ggplot2::ggsave(file, plot = plotInput(), device = device(width = 725, height = 1000, res = 120))
    })

  output$download_plot_ts <- downloadHandler(
    filename = function(){
      paste0("Zeitreihe", "_", input$ts_period[1], "_", input$ts_period[2], ".png")
    },
    content = function(file) {
      device <- function(..., width, height, res) {
        grDevices::png(..., width = width, height = height,
      ...
      ggplot2::ggsave(file, plot = plotInput_ts(), device = device(width = 1000, height = 725, res = 120))
    })
