...
create_herkunft_table <- function(df, start_date, end_date){
  ...
  herkunft_table <- df_processed %>%
    mutate(Kalenderwoche = isoweek(date),
           Jahr = lubridate::isoyear(date)) %>%
    group_by(Jahr, Kalenderwoche) %>%
    mutate(Start = format(min(date), "%Y-%m-%d"),
    ...
    mutate(
      `Alle Anrufer*innen` = rowSums(across(ends_with(" Anrufer*innen")), na.rm = TRUE),
      `Alle Anrufe` = rowSums(across(ends_with(" Anrufe")), na.rm = TRUE),
      `Alle Erstanrufer*innen` = rowSums(across(ends_with(
        " Erstanrufer*innen"
      )), na.rm = TRUE)
    ) %>%
