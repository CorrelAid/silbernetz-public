...
heatmap_laender <- function(
  ...
  if (nrow(numbers_geo) == 0) {
    final_plot <- ggplot(data.frame(x = 1:100, y = 1:100)) +
      ...
      annotate("text", x = 50, y = 50, label = "No data available", size = 7) +
      theme_void()
    return(final_plot)
  }
  ...
  start_date <- paste(
    str_sub(start_date, 9, 10),
    str_sub(start_date, 6, 7),
    str_sub(start_date, 1, 4),
    sep = "."
  )
  end_date <- paste(
    str_sub(end_date, 9, 10),
    str_sub(end_date, 6, 7),
    str_sub(end_date, 1, 4),
    sep = "."
  )
  ...
  df_germany_lev1 <- df_germany_lev1 %>%
    mutate(
      !!as.symbol(variable) := ifelse(
        !!as.symbol(variable) == 0,
        NA,
        !!as.symbol(variable)
      )
    )
  ...
  coord_names <- df_germany_lev1 %>%
    ...
    group_by(id) %>%
    summarise(
      Bundesland_kuerzel = unique(Bundesland_kuerzel),
    ...
    mutate(
      centr_lat = case_when(
        # some manual adjustments of label positions
  ...
  plot_map_bdl <- ggplot(df_germany_lev1) +
    ...
    scale_fill_gradient(
      ...
      na.value = "grey90"
    ) + # corporate_lightblue, corporate_blue
    theme(
  ...
  if (start_date != end_date) {
    # add title, which either refers to a period or single day
    plot_map_bdl <- plot_map_bdl +
      labs(
        title = paste0(
          "Anzahl der ",
          variable_label,
