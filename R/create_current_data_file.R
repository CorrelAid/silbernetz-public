...
create_current_data <- function(
  end_date,
  write_path = "../data/raw/current_data.csv"
) {
  df_complete1 <- download_data(
    "Numbers",
    start_date = "2020-01-01",
    end_date = "2021-07-26"
  )
  # loading is split, because access is limited to 100,000 rows at a time
  df_complete2 <- download_data(
    "Numbers",
    start_date = "2021-07-27",
    end_date = end_date
  )
  df_complete <- rbind(df_complete1, df_complete2) %>%
